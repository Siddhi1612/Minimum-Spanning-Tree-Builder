...
def findMST(graph, userAlgorithm):
    if userAlgorithm == "kruskal":
        ...
        primType = input("Prim's with matrix or graph input: ")
        if primType == "matrix":
...
#main program
mst_edges = findMST(Graph2.graph, Graph2.userAlgorithm)
userMST = nx.Graph()
userMST.add_nodes_from(Graph2.graph.nodes())
userMST.add_edges_from(mst_edges)
Graph2.drawGraph(userMST)